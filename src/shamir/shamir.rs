...
// use super::tables::{LOG_TABLE, EXP_TABLE};
...
// impl<'a> Polynomial {
//     pub fn new(intercept: u8, degree: u8) -> Polynomial {
//         let mut polynomial = Polynomial {
//             coefficients: Vec::with_capacity(degree as usize + 1),
//         };
...
//                 continue;
//             }
//             let num = x.wrapping_add(x_samples[j]);
//             let denom = x_samples[i].wrapping_add(x_samples[j]);
//             let term = num.wrapping_div(denom);
//             basis = multiply(basis, term);
//         }
//         let group = y_samples[i].wrapping_mul(basis);
...
//     secret.iter().enumerate().for_each(|(index, &val)| {
...
//             let x: u8 = (x_coordinates[i] as u8) + 1;
//             let y = p.evaluate(x);
//             output[i][index] = y;
//         }
//     });
